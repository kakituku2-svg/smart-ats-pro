...
func _apply_variant_profile() -> void:
    match variant_id:
        ...
        &"glow_leech":
            ...
            chase_radius = 8.5
            attack_radius = 2.25
            attack_cooldown = 1.70
        _:
            variant_id = &"guard_bug"
            display_name = "ガードバグ"
            max_health = 3
            move_speed = 3.8
            chase_radius = 9.0
...
func _physics_process(delta: float) -> void:
    ...
    if distance <= chase_radius and distance > 0.05:
        ...
        if variant_id == &"wind_stinger":
            desired_speed *= 0.88 + sin(_time * 4.6) * 0.12
...
func _perform_attack(player: PlayerController, direction: Vector3) -> void:
    match variant_id:
        &"glow_leech":
            player.apply_slow(0.52, 1.8)
            player.add_camera_impulse(Vector2(3.0, -2.0))
            AudioManager.play_event(&"warning")
        &"wind_stinger":
            if player.take_damage(1, -direction * 1.35):
                player.add_camera_impulse(Vector2(7.0, -4.0))
                AudioManager.play_event(&"warning")
        _:
            if player.take_damage(1, -direction):
                AudioManager.play_event(&"warning")
...
func trigger_pulse(target: Vector3) -> bool:
    if _dead:
        return false
    if global_position.distance_to(target) > 4.2:
        return false
    health = maxi(0, health - 1)
    _stun = 0.80 if variant_id == &"wind_stinger" else 0.62
    AudioManager.play_event(&"enemy_hit")
...
func _defeat() -> void:
    ...
    AudioManager.play_event(&"enemy_down")
...
func _flash_hit() -> void:
    ...
    tween.tween_property(_visual, "scale", Vector3.ONE * 1.18, 0.06)
...
func _animate_visual() -> void:
    if _visual != null:
        ...
        if variant_id == &"glow_leech":
            _visual.rotation.z = sin(_time * 2.2) * 0.08
    if _eye != null:
...
func _build_guard_bug_visual() -> void:
    var shell := _sphere("Shell", Vector3.ZERO, Vector3(1.25, 0.72, 1.0), Color(0.14, 0.17, 0.20, 1.0), false, 0.52)
    ...
    for side in [-1.0, 1.0]:
        for row in [-0.28, 0.22]:
            _limb("Leg", Vector3(side * 0.58, -0.18, row), Vector3(0, 0, side * -58.0), 0.72, Color(0.09, 0.11, 0.13, 1.0))
...
func _build_wind_stinger_visual() -> void:
    _sphere("Core", Vector3(0, 0.10, 0), Vector3(0.88, 0.72, 1.12), Color(0.08, 0.16, 0.22, 1.0), false, 0.44)
    ...
    for side in [-1.0, 1.0]:
        ...
        wing_mesh.size = Vector3(0.18, 0.62, 0.56)
        wing.mesh = wing_mesh
        wing.position = Vector3(side * 0.43, 0.10, 0.06)
        wing.rotation = Vector3(0.05, 0, side * 0.78)
        wing.material_override = _mat(accent_color.lightened(0.18), true)
        _visual.add_child(wing)
        _limb("Stinger", Vector3(side * 0.16, 0.56, 0.15), Vector3(-18, 0, side * -12.0), 0.42, accent_color.darkened(0.12))
    var tail := _limb("TailSpike", Vector3(0, 0.0, 0.52), Vector3(75, 0, 0), 0.62, Color(0.18, 0.24, 0.28, 1.0))
    tail.scale = Vector3(0.78, 1.0, 0.78)

func _build_glow_leech_visual() -> void:
    _sphere("Body", Vector3(0, -0.02, 0), Vector3(1.52, 0.66, 1.25), Color(0.08, 0.13, 0.16, 1.0), false, 0.54)
    ...
    for i in range(5):
        ...
        var tendril := _limb("Tendril", Vector3(sin(angle) * 0.42, -0.22, cos(angle) * 0.34), Vector3(62, angle, sin(angle) * 18.0), 0.58, accent_color.darkened(0.22))
        tendril.scale = Vector3(0.70, 1.0, 0.70)
    for side in [-1.0, 1.0]:
        _sphere("GlowNode", Vector3(side * 0.42, 0.11, 0.18), Vector3(0.70, 0.70, 0.70), accent_color.lightened(0.20), true, 0.09)
...
func _limb(node_name: String, pos: Vector3, rotation_degrees_value: Vector3, height: float, color: Color) -> MeshInstance3D:
    ...
    mesh.top_radius = 0.038
    mesh.bottom_radius = 0.062
    mesh.height = height
...
func _mat(color: Color, emissive: bool) -> StandardMaterial3D:
    ...
    mat.roughness = 0.62 if emissive else 0.88
    if emissive:
